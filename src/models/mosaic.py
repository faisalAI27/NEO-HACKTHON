...
class MOSAIC(nn.Module):
    ...
    def __init__(self, config):
        ...
        input_dims = config[
            "data_dims"
        ]  # Dict with input shapes e.g. {'rna': 3000, 'wsi': 1024}
    ...
    def forward(self, batch, return_attention: bool = False):
        ...
        if return_attention:
            fused, cross_attn_weights = self.fusion(
                encoded_modalities, return_attention=True
            )
        else:
        ...
        if return_attention:
            ...
            all_attn = {"cross_modal": cross_attn_weights, **intra_modal_attn}
