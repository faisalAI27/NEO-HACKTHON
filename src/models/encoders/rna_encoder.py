...
class MLPEncoder(nn.Module):
    ...
    def __init__(
        self, input_dim, hidden_dim=512, output_dim=256, dropout=0.3, num_layers=2
    ):
        super().__init__()
        layers = []
        ...
        self.output_projection = nn.Sequential(
            nn.Linear(curr_dim, output_dim), nn.LayerNorm(output_dim)
        )
...
class PathwayAttentionEncoder(nn.Module):
    ...
    def __init__(
        ...
        # Output Projection
        self.output_projection = nn.Sequential(
            nn.Linear(d_model, output_dim), nn.LayerNorm(output_dim)
        )
