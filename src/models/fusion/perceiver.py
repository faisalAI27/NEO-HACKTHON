...
class PerceiverFusion(nn.Module):
    ...
    """
    def __init__(self, modal_dims, num_latents=32, latent_dim=256, num_cross_heads=4, num_self_heads=4, num_layers=2, dropout=0.1):
        """
        Args:
        ...
        for _ in range(num_layers):
            layer = nn.ModuleDict({
                'attn': nn.MultiheadAttention(embed_dim=latent_dim, num_heads=num_self_heads, batch_first=True, dropout=dropout),
        ...
        self.output_pool = nn.Linear(latent_dim * num_latents, latent_dim) # Flatten and project? Or just Mean?
