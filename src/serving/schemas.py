...
class ClinicalData(BaseModel):
    ...
    age: Optional[float] = Field(
        None, ge=0, le=120, description="Patient age at diagnosis in years"
    )

    gender: Optional[GenderEnum] = Field(None, description="Patient gender")

    tumor_stage: Optional[TumorStageEnum] = Field(None, description="AJCC tumor stage")

    # Additional clinical features (extensible)
    hpv_status: Optional[bool] = Field(
        None, description="HPV positive status (for HNSC)"
    )

    smoking_history: Optional[int] = Field(
        None, ge=0, description="Pack-years of smoking history"
    )

    alcohol_history: Optional[bool] = Field(None, description="History of alcohol use")

    tumor_site: Optional[str] = Field(
        None, description="Primary tumor site (e.g., 'oral cavity', 'oropharynx')"
    )
...
class OmicsData(BaseModel):
    ...
    gene_names: Optional[List[str]] = Field(
        None, description="Gene symbols corresponding to expression values"
    )
    ...
    probe_ids: Optional[List[str]] = Field(
        None, description="CpG probe IDs corresponding to methylation values"
    )

    # Mutation data
    mutated_genes: Optional[List[str]] = Field(
        None, description="List of mutated gene symbols"
    )
    ...
    driver_mutations: Optional[List[str]] = Field(
        None, description="List of known driver gene mutations present"
    )
...
class WSIData(BaseModel):
    ...
    slide_id: Optional[str] = Field(
        None, description="Slide identifier for server-side feature extraction"
    )
    ...
    coordinates: Optional[List[List[int]]] = Field(
        None, description="Patch coordinates (N x 2 for x, y positions)"
    )

    class Config:
        schema_extra = {"example": {"slide_id": "TCGA-CV-7432-01A-01-TS1"}}
...
class PredictionRequest(BaseModel):
    ...
    patient_id: Optional[str] = Field(
        None, description="Optional patient identifier for tracking"
    )

    clinical: Optional[ClinicalData] = Field(None, description="Clinical data")

    omics: Optional[OmicsData] = Field(
        None, description="Omics data (expression, methylation, mutations)"
    )

    wsi: Optional[WSIData] = Field(None, description="Whole slide image data")
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "clinical": {"age": 62.5, "gender": "male", "tumor_stage": "stage iii"},
...
class PredictionResponse(BaseModel):
    ...
    patient_id: Optional[str] = Field(
        None, description="Patient identifier from request"
    )
    ...
    risk_group: RiskGroupEnum = Field(..., description="Risk stratification category")
    ...
    attention_maps: Optional[Dict[str, Any]] = Field(
        None, description="Attention weights for interpretability (if requested)"
    )

    model_version: Optional[str] = Field(
        None, description="Model checkpoint identifier"
    )
    ...
    class Config:
        ...
        schema_extra = {
            "example": {
                ...
                "risk_score": 0.75,
                "risk_group": "high",
                "survival_probabilities": {365: 0.82, 730: 0.65, 1095: 0.48},
