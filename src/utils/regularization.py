...
class RegularizationModule:
    ...
    def mixup_data(
        self, batch: Dict[str, torch.Tensor], alpha: float = 0.2
        ...
        """
        if alpha <= 0:
            return batch, torch.ones(
                batch["time"].shape[0], device=batch["time"].device
            )
        ...
        for key, value in batch.items():
            ...
            if isinstance(value, torch.Tensor) and value.dim() >= 1:
                ...
                mixed_batch[key] = (
                    lam_expanded * value + (1 - lam_expanded) * value[indices]
                )
            else:
        ...
        mixed_batch["event"] = torch.where(lam > 0.5, event1, event2)

        # Alternative: Use event from patient with shorter time (more informative)
        shorter_idx = time1 < time2
        mixed_batch["event"] = torch.where(shorter_idx, event1, event2)
...
class SurvivalRegularizer(nn.Module):
    ...
    def temporal_smoothness(
        self, risk_scores: torch.Tensor, times: torch.Tensor
    ) -> torch.Tensor:
        """
...
def get_regularized_optimizer(
    ...
    for name, param in model.named_parameters():
        ...
        is_no_decay = any(
            nd in name.lower() for nd in ["bias", "layernorm", "layer_norm", "ln"]
        )
    ...
    param_groups = [
        {"params": decay_params, "weight_decay": config.weight_decay, "lr": lr},
