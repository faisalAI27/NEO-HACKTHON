...
class StratifiedSurvivalSampler(Sampler):
    ...
    def _create_strata(self):
        ...
        if self.stratify_by_time and self.times is not None:
            ...
            time_bins = np.percentile(
                self.times, np.linspace(0, 100, self.n_time_bins + 1)[1:-1]
            )
        ...
        self.strata_indices = {
            s: np.where(self.strata_labels == s)[0] for s in unique_strata
        }
        ...
        if self.oversample_events and self.event_ratio < 0.3:
            ...
            for s in self.strata_indices:
                ...
                is_event_stratum = any(
                    self.events[i] == 1 for i in self.strata_indices[s][:5]
                )
                if is_event_stratum:
    ...
    def __iter__(self) -> Iterator[List[int]]:
        ...
        shuffled_strata = {
            s: rng.permutation(idx).tolist() for s, idx in self.strata_indices.items()
        }
...
def create_stratified_cv_splits(
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(patient_ids, strata)):
        ...
        print(
            f"Fold {fold}: Train {len(train_idx)} ({train_events} events), "
