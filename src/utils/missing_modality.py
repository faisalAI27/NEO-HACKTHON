...
class MissingModalityHandler:
    ...
    def detect_missing(
        self, batch: Dict[str, torch.Tensor], threshold: float = 1e-6
    ...
    def impute(
        self, batch: Dict[str, torch.Tensor], masks: Dict[str, torch.Tensor]
    ) -> Dict[str, torch.Tensor]:
        """
        ...
        for modality, tensor in batch.items():
            ...
            cfg = self.modality_configs[modality]
            mask = masks.get(
                modality, torch.ones(tensor.shape[0], dtype=torch.bool, device=device)
            )
            ...
            if mask.all():
                ...
                impute_values = (
                    torch.tensor(
                        cfg.imputation_value, dtype=tensor.dtype, device=device
                    )
                    .unsqueeze(0)
    ...
    def apply_dropout(
        ...
        if not training or self.config.dropout_rate <= 0:
            ...
            masks = {
                m: torch.ones(
                    batch[m].shape[0], dtype=torch.bool, device=batch[m].device
                )
                for m in batch
                if m in self.modality_configs
            }
            return batch, masks
    ...
    def get_modality_masks_tensor(self, masks: Dict[str, torch.Tensor]) -> torch.Tensor:
        """
...
class ModalityMaskCollator:
    ...
    def __init__(
        self, handler: MissingModalityHandler, modality_keys: List[str] = None
    ):
        """
...
class LearnedImputationNetwork(nn.Module):
    ...
    def __init__(
        self, modality_dims: Dict[str, int], hidden_dim: int = 256, n_layers: int = 2
    ):
        """
        ...
        self.encoders = nn.ModuleDict(
            {
                name: nn.Sequential(
                    nn.Linear(dim, hidden_dim), nn.ReLU(), nn.Dropout(0.1)
                )
        ...
        for _ in range(n_layers):
            fusion_layers.extend(
                [
                    nn.Linear(
                        fusion_input if len(fusion_layers) == 0 else hidden_dim,
                        hidden_dim,
                    ),
                    nn.ReLU(),
                    nn.Dropout(0.1),
        ...
        self.decoders = nn.ModuleDict(
            {name: nn.Linear(hidden_dim, dim) for name, dim in modality_dims.items()}
        )
