...
class ChunkedWSIProcessor:
    ...
    def _adjust_chunk_size_for_memory(self):
        ...
        if self.device == "cuda" and torch.cuda.is_available():
            ...
            print(
                f"Adjusted chunk size to {self.config.chunk_size} based on GPU memory"
            )

    def _tile_generator(
        self, slide, coordinates: List[Tuple[int, int]], read_size: int
    ...
    def _process_chunk(self, tiles: List[np.ndarray], chunk_idx: int) -> np.ndarray:
        """
    ...
    def _save_chunk(
        self, features: np.ndarray, chunk_idx: int, slide_name: str
    ) -> Path:
    ...
    def _load_and_merge_chunks(self, chunk_paths: List[Path]) -> np.ndarray:
    ...
    def process_slide(
        ...
        if coordinates is None:
            ...
            coordinates, read_size = tiler.get_tile_coordinates(
                slide, mask, mask_downsample
            )
        ...
        with h5py.File(output_path, "w") as f:
            f.create_dataset(
                "features", data=final_features, compression="gzip", compression_opts=4
            )
    ...
    def process_batch(
        self, slide_paths: List[Path], output_dir: Path, force: bool = False
    ) -> Dict[str, Path]:
        """
...
class ChunkedFeatureLoader:
    ...
    def __init__(self, feature_dir: Path, max_tiles: int = 4000, cache_size: int = 10):
        """
    ...
    def load_features(
        self, patient_id: str, pad_to: Optional[int] = None
        ...
        if pad_to is not None and actual_n_tiles < pad_to:
            padding = np.zeros(
                (pad_to - actual_n_tiles, features.shape[1]), dtype=np.float32
            )
