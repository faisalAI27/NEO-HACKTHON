...
class MOSAICTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        inputs = {k: v for k, v in batch.items() if k not in ['time', 'event', 'case_id']}
        time = batch['time']
        event = batch['event']
    ...
    def on_validation_epoch_end(self):
        ...
        all_risk = torch.cat([x['risk_scores'] for x in self.validation_step_outputs]).numpy().flatten()
        all_time = torch.cat([x['time'] for x in self.validation_step_outputs]).numpy().flatten()
        all_event = torch.cat([x['event'] for x in self.validation_step_outputs]).numpy().flatten()
