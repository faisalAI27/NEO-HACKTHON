        ...
        with torch.no_grad():
            for k, v in self.reference_batch.items():
                ...
                if isinstance(v, torch.Tensor):
                    ...
                    ref_sample = v[0:1] # (1, ...)
                    batch[k] = ref_sample.repeat(N, *([1]*(v.ndim-1))).to(self.device)
                elif isinstance(v, dict):
                     batch[k] = {}
                     for sub_k, sub_v in v.items():
                         ref_sample = sub_v[0:1]
                         batch[k][sub_k] = ref_sample.repeat(N, *([1]*(sub_v.ndim-1))).to(self.device)
            
            # Set target
            # x_input is numpy
            batch[self.target_modality] = torch.tensor(x_input, dtype=torch.float32).to(self.device)
            
            # Forward
            logits = self.model(batch)
            return logits.cpu().numpy().flatten()
            
def run_shap(checkpoint_path, modality='clinical', fold=0, num_background=10, num_test=5):
    ...
    train_loader = DataLoader(train_dataset, batch_size=num_background, shuffle=True, collate_fn=custom_collate)
    
    val_dataset = MultiModalSurvivalDataset(data_dir, split_path, 'val', fold)
    val_loader = DataLoader(val_dataset, batch_size=num_test, shuffle=False, collate_fn=custom_collate)
    ...
    for i in range(num_test):
        ...
        shap_values = explainer.shap_values(x_explain, nsamples=100) # Limited nsamples for speed
        explanations.append(shap_values[0]) # shap_values is list if output is list? wrapper returns (N,). So shap_values is (1, Feats).
    ...
    plt.figure()
    shap.summary_plot(explanations, features=val_batch[modality][:num_test].numpy(), feature_names=feature_names, show=False)
